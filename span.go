...
//
// Logs duration both ways: `duration=` is time.Duration.String()'s
// human-formatted form ("1.234ms", "820µs") for raw-log readability;
// `duration_ns=` is the same value as a plain integer nanosecond
// count, so a downstream ingest pattern (a trailing key=value regex,
// same shape as trace_id_pattern) can extract it straight into a
// numeric column without reimplementing Go's unit-suffix parsing.
func (s *Span) End() {
	...
	log.Printf("span done name=%s trace_id=%s span_id=%s parent_span_id=%s duration=%s duration_ns=%d status=%s",
		s.name, s.traceID, s.spanID, s.parentID, duration, duration.Nanoseconds(), status)
}
